...
from .cmran_modules import New_Audio_Guided_Attention
from .cmran_modules import EncoderLayer, Encoder, DecoderLayer, Decoder
...
class WeaklyLocalizationModule(nn.Module):
    def __init__(self, input_dim, category_num):
        ...
        self.event_classifier = nn.Linear(self.hidden_dim, category_num)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, fused_content):
        fused_content = fused_content.transpose(0, 1) # [B, 10, 256]
        max_fused_content, _ = fused_content.max(1) # [B, 256]
        # confident scores
        is_event_scores = self.classifier(fused_content) # [B, 10, 1]
        # classification scores
        raw_logits = self.event_classifier(max_fused_content)[:, None, :]  # [B, 1, 29]
        # fused
        fused_logits = is_event_scores.sigmoid() * raw_logits # [B, 10, 29]
        # Training: max pooling for adapting labels
        logits, _ = torch.max(fused_logits, dim=1) # [B, 29]
        event_scores = self.softmax(logits) # [B, 29]
...
class weak_main_model(nn.Module):
    def __init__(self, vis_fea_type='vgg', category_num=28):
        ...
        self.video_input_dim = 1024 # for vggsound-avel100k
        ...
        self.localize_module = WeaklyLocalizationModule(self.d_model, self.category_num)
    ...
    def forward(self, visual_feature, audio_feature):
        ...
        visual_feature = visual_feature.permute(0, 1, 3, 4, 2) # for vggsound-avel100k
...
class supv_main_model(nn.Module):
    def __init__(self, vis_fea_type, category_num=28):
        ...
        if self.vis_fea_type == 'vgg':
            ...
            self.video_input_dim = 1024 # for resnet feature
        self.video_fc_dim = 512
        self.d_model = 256
        self.v_fc = nn.Linear(self.video_input_dim, self.video_fc_dim)
        self.relu = nn.ReLU()
    ...
    def forward(self, visual_feature, audio_feature):
        ...
        audio_feature = audio_feature.transpose(1, 0).contiguous()
        if self.vis_fea_type == 'resnet':
            visual_feature = visual_feature.permute(0, 1, 3, 4, 2)
