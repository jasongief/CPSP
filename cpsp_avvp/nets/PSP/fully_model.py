...
class LSTM_A_V(nn.Module):
    def __init__(self, a_dim, v_dim, hidden_dim):
    ...
    def init_hidden(self, a_fea, v_fea):
        ...
        # hidden_a = (torch.zeros(2, bs, self.hidden_dim), torch.zeros(2, bs, self.hidden_dim))
        # hidden_v = (torch.zeros(2, bs, self.hidden_dim), torch.zeros(2, bs, self.hidden_dim))
        hidden_a = (torch.zeros(2, bs, self.hidden_dim).cuda(), torch.zeros(2, bs, self.hidden_dim).cuda())
        hidden_v = (torch.zeros(2, bs, self.hidden_dim).cuda(), torch.zeros(2, bs, self.hidden_dim).cuda())
        return hidden_a, hidden_v

    def forward(self, a_fea, v_fea):
        # a_fea, v_fea: [bs, 10, 128/512]
...
if __name__ == "__main__":
    model = LSTM_A_V(a_dim=512, v_dim=512, hidden_dim=256)
    input_a = torch.randn(8, 10, 512)
    input_v = torch.randn(8, 10, 512)
    ...
    output_a, output_v = model(input_a, input_v)
    print('output_a.shape: ', output_a.shape)
    print('output_v.shape: ', output_v.shape)
